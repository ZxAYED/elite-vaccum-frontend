  ...
  return (
    <section className="space-y-24 bg-[#F9F9F9]">
      ...
      <div className="text-center space-y-10 py-12 px-8 md:px-0">
        ...
        <div className="grid md:grid-cols-2 gap-8 mt-16 lg:mt-20 max-w-360 mx-auto">
          {stats.map((item) => {
            const Icon = item.icon;
            return (
              <div
                key={item.label}
                className="rounded-sm bg-white p-6 shadow-sm border text-left"
              >
      ...
      <div className="text-center space-y-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-360 mx-auto px-4">
          {team.map((member) => (
            <div
              key={member.name}
              className="bg-white border rounded-xl p-4 md:p-6 text-center shadow-sm"
            >
      ...
      <div className="bg-muted py-10 px-8 md:px-0">
        <div className="max-w-360 mx-auto grid grid-cols-1 md:grid-cols-4 gap-6">
          {badges.map((badge) => {
            const Icon = badge.icon;
            return (
              <div
                key={badge.text}
                className="bg-white flex items-center justify-around md:justify-center gap-4 rounded-lg  py-4 md:py-6 text-center shadow-sm text-xl font-medium"
              >
